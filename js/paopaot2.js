...
paopaot2Obj.prototype.num=30;
paopaot2Obj.prototype.init=function(){
...
var flag=0;
paopaot2Obj.prototype.draw=function(ctx,r,g,b,m){
    for( var i=0; i<this.num;i++){
        if(this.alive[i]){
            ctx.save();
            ctx.beginPath();
...
paopaot2Obj.prototype.born=function(i){
    this.x[i]=Math.random()*cavW;
    this.y[i]=Math.random()*cavH*3/5;
    this.l[i]=Math.random()*120;
    this.spd[i]=Math.random()*0.001+0.005;
    this.alive[i]=true;
}
function paopaoMonitort2(){
    var num=0;
    for(var i=0; i<paopaot2.num;i++){
        if(paopaot2.alive[i]) num++;
    }
    if(num<95){
        sendPaopao();
        return;
